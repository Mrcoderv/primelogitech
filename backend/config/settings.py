...
# ── Allowed Hosts ────────────────────────────────────────────
# Support both comma-separated string and single value
...
MIDDLEWARE = [
    'config.middleware.HealthCheckMiddleware',       # ← must be first – handles Render probes before ALLOWED_HOSTS check
...
# ── Database ─────────────────────────────────────────────────
# Render PostgreSQL via DATABASE_URL, fallback to SQLite
DATABASES = {
...
if not DEBUG:
    ...
    SECURE_HSTS_SECONDS = 31536000  # 1 year; start with 600 if unsure
