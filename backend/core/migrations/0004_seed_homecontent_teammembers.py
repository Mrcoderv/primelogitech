...
def seed_home_content_and_team(apps, schema_editor):
	...
	HomeContent.objects.get_or_create(
		why_title='Why partner with Prime Logitech?',
		defaults={
			...
			'client_success_description': "Don't just take our word for it. Hear what our partners have to say about working with Prime Logitech.",
		}
	)

	team_members = [
		('Prasiddha Gyawali', 'CEO & Founder', 'Visionary leader driving the strategic direction of Prime Logitech to deliver exceptional digital experiences.', 0),
...
def unseed_home_content_and_team(apps, schema_editor):
	...
	HomeContent.objects.filter(why_title='Why partner with Prime Logitech?').delete()
