...
import { Users, Plus, Edit3, Trash2, Loader2, X } from 'lucide-react';
...
export default function AdminTeam() {
  ...
  const [form, setForm] = useState({
    name: '', role: '', bio: '', linkedin_url: '', github_url: '', twitter_url: '', order: 0, is_active: true,
  });

  const load = useCallback(async () => {
    setLoading(true);
    setError('');
    try { setItems(await fetchAdminTeam()); }
  ...
  function resetForm() {
    setForm({ name: '', role: '', bio: '', linkedin_url: '', github_url: '', twitter_url: '', order: 0, is_active: true });
  }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editing === 'new') await createAdminTeam(payload);
      else await updateAdminTeam(editing, payload);
      setEditing(null);
      await load();
  ...
  async function handleDelete(id) {
    ...
    try { await deleteAdminTeam(id); await load(); }
    ...
    <div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">{editing === 'new' ? 'New Team Member' : 'Edit Team Member'}</h2>
          ...
          <div>
            ...
            <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} rows={3} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:ring-2 focus:ring-cyan-500" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-gray-400 mb-1">LinkedIn URL</label>
              <input value={form.linkedin_url} onChange={(e) => setForm({ ...form, linkedin_url: e.target.value })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:ring-2 focus:ring-cyan-500" />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.twitter_url} onChange={(e) => setForm({ ...form, twitter_url: e.target.value })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:ring-2 focus:ring-cyan-500" />
            </div>
          </div>
          <div className="flex items-center gap-6">
            ...
            <label className="flex items-center gap-2 text-sm text-gray-300 pt-4">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <p className="text-sm">Click "Add Member" to create one.</p>
        </div>
      ) : (
